        ...
        return (
          <React.Fragment key={item.id}>
            {!!item?.subMenu ? (
              ...
              >
                {item.name} <BsChevronDown size={16} />
                {showCatMenu && (
                    ...
                    {subMenuData.map((submenu) => {
                      return (
                        <Link key={submenu.id} href={"/"} onClick={()=>setShowCatMenu(false)}>
                          <li className="h-12 flex justify-between items-center px-3 hover:bg-black/[0.03] rounded-md">{submenu.name}<span className="opacity-50 text-sm">78</span></li>
                        </Link>
                      );
                    })}
                  </ul>
